...
while (($row = fgetcsv($file_handle, 0, ',')) !== false) {
    ...
    // Parse row (ID, Product Name, SKU, Category, Price, Stock, Created Date)
    ...
    if ($check_result->num_rows > 0) {
        ...
        $stmt->bind_param("ssdis", $name, $category, $price, $stock, $sku);
    } else {
        // Insert new product
        $stmt = $conn->prepare("INSERT INTO products (name, sku, category, price, stock, created_at) VALUES (?, ?, ?, ?, ?, NOW())");
    ...
    if ($stmt->execute()) {
        ...
        $failed_count++;
        $errors[] = "Row $imported_count: " . $stmt->error;
    }
...
// Log the import
$log_stmt = $conn->prepare("INSERT INTO export_import_log (user_id, action, file_format, record_count, status) VALUES (?, 'import', 'csv', ?, ?)");
$log_status = $failed_count === 0 ? 'success' : 'partial';
$log_stmt->bind_param("iis", $user_id, $imported_count, $log_status);
$log_stmt->execute();
$log_stmt->close();

echo json_encode([
    "status" => $failed_count === 0 ? "success" : "partial",
    "message" => "Import completed: $imported_count records added/updated, $failed_count failed",

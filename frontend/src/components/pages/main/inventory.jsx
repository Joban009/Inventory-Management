...
const getStockMeta = (stock) => {
  if (stock >= 50)
    return { stockLabel: "Good", stockPct: 100, barClass: "bg-emerald-500" };
  if (stock >= 15)
    return { stockLabel: "Low", stockPct: 40, barClass: "bg-amber-500" };
...
const Inventory = () => {
  ...
  const rows = (products || []).filter(Boolean).map((p) => {
    ...
    const { stockLabel, stockPct, barClass } = getStockMeta(
      Number(p?.stock ?? 0),
    );
    ...
    return {
      ...
      barClass,
      catClass:
        catClassMap[p?.category] ?? "bg-gray-50 text-gray-700 ring-gray-700/10",
  ...
  const handleItemAdded = async () => {
    await fetchProducts(); // always reload from backend
    setPage(1);
  };
  return (
        ...
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
          ...
          <div className="flex flex-col items-center justify-between gap-3 border-t border-gray-100 px-5 py-4 sm:flex-row">
            <p className="text-xs text-gray-500">
              Showing {rows.length === 0 ? 0 : (page - 1) * ITEMS_PER_PAGE + 1}{" "}
              to {Math.min(page * ITEMS_PER_PAGE, rows.length)} of {rows.length}{" "}
              results
            </p>
